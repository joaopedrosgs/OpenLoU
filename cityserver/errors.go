package cityserver

const (
	BadCityID                    = "Bad CityID"
	BadXValue                    = "Bad X value"
	BadYValue                    = "Bad Y value"
	BadConstructionType          = "Bad construction type"
	Success                      = "success"
	FailedNewConstruction        = "failed to create construction"
	FailedNewConstructionUpgrade = "failed to create upgrade to the new construction"
	TileInUse                    = "tile already in use"
	FailedGetConstructions       = "failed to get constructions"
)

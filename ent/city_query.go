...
func (cq *CityQuery) QueryConstructions() *ConstructionQuery {
	query := (&ConstructionClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithConstructions tells the query-builder to eager-load the nodes that are connected to
// the "constructions" edge. The optional arguments are used to configure the query builder of the edge.
...
func (cq *CityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*City, error) {
	...
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *City, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withConstructions; query != nil {
		if err := cq.loadConstructions(ctx, query, nodes,
			func(n *City) { n.Edges.Constructions = []*Construction{} },
			func(n *City, e *Construction) { n.Edges.Constructions = append(n.Edges.Constructions, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withQueue; query != nil {
		if err := cq.loadQueue(ctx, query, nodes,
			func(n *City) { n.Edges.Queue = []*Queue{} },
			func(n *City, e *Queue) { n.Edges.Queue = append(n.Edges.Queue, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CityQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*City, init func(*City), assign func(*City, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*City)
	for i := range nodes {
		if nodes[i].user_cities == nil {
			continue
		}
		fk := *nodes[i].user_cities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_cities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CityQuery) loadConstructions(ctx context.Context, query *ConstructionQuery, nodes []*City, init func(*City), assign func(*City, *Construction)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "city_constructions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "city_constructions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CityQuery) loadQueue(ctx context.Context, query *QueueQuery, nodes []*City, init func(*City), assign func(*City, *Queue)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*City)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.city_queue
		if fk == nil {
			return fmt.Errorf(`foreign-key "city_queue" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "city_queue" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

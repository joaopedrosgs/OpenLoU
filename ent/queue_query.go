...
func (qq *QueueQuery) QueryCity() *CityQuery {
	query := (&CityClient{config: qq.config}).Query()
...
func (qq *QueueQuery) QueryConstruction() *ConstructionQuery {
	query := (&ConstructionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QueueQuery) Clone() *QueueQuery {
	...
	return &QueueQuery{
		...
		withCity:         qq.withCity.Clone(),
		withConstruction: qq.withConstruction.Clone(),
...
// WithConstruction tells the query-builder to eager-load the nodes that are connected to
// the "construction" edge. The optional arguments are used to configure the query builder of the edge.
...
func (qq *QueueQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Queue, error) {
	...
	if query := qq.withCity; query != nil {
		if err := qq.loadCity(ctx, query, nodes, nil,
			func(n *Queue, e *City) { n.Edges.City = e }); err != nil {
			return nil, err
		}
	}
	if query := qq.withConstruction; query != nil {
		if err := qq.loadConstruction(ctx, query, nodes, nil,
			func(n *Queue, e *Construction) { n.Edges.Construction = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QueueQuery) loadCity(ctx context.Context, query *CityQuery, nodes []*Queue, init func(*Queue), assign func(*Queue, *City)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Queue)
	for i := range nodes {
		if nodes[i].city_queue == nil {
			continue
		}
		fk := *nodes[i].city_queue
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "city_queue" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (qq *QueueQuery) loadConstruction(ctx context.Context, query *ConstructionQuery, nodes []*Queue, init func(*Queue), assign func(*Queue, *Construction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Queue)
	for i := range nodes {
		if nodes[i].construction_queue == nil {
			continue
		}
		fk := *nodes[i].construction_queue
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "construction_queue" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

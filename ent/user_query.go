...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCities; query != nil {
		if err := uq.loadCities(ctx, query, nodes,
			func(n *User) { n.Edges.Cities = []*City{} },
			func(n *User, e *City) { n.Edges.Cities = append(n.Edges.Cities, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadCities(ctx context.Context, query *CityQuery, nodes []*User, init func(*User), assign func(*User, *City)) error {
	...
	for _, n := range neighbors {
		fk := n.user_cities
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_cities" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_cities" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func testUsersExists(t *testing.T) {
	...
	e, err := UserExists(ctx, tx, o.Email)
	if err != nil {
...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.Email)
	if err != nil {
		t.Error(err)
	}
...
var (
	userDBTypes = map[string]string{`AllianceName`: `character varying`, `AllianceRank`: `character varying`, `CreatedAt`: `timestamp without time zone`, `Darkwood`: `integer`, `Diamonds`: `integer`, `Email`: `character varying`, `Gold`: `integer`, `Name`: `character varying`, `Password`: `character varying`, `Rank`: `integer`, `Runestone`: `integer`, `Trueseed`: `integer`, `UpdatedAt`: `timestamp without time zone`, `Veritium`: `integer`}
	_           = bytes.MinRead
)
